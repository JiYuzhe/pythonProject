...
# review 首先是将epsilon统一起来
epsilon = 0.00001
epsilon_Q = 0.01
epsilon_G = 0.001
# review 这个可以直接拿来用，用来判断第一个条件，这里没有任何修改
...
# review 第三个条件的判断
# TODO 这个条件的判断改变了，其中constraint应该都不变，主要是obj变了，变得更简单了，只需要判断nx是否在[1-epsilon, 3+epsilon]之间即可
def milp_final_outputs(x_min, x_max, W, b):
    ...
    # TODO 这里的objective变了，变成求nx的最大值和最小值了
    lp_f = np.zeros((1, all_variable_num)).squeeze()
    # review 这是目标要用的，前面的variable系数都是0，最后两个输出神经元相减的目标值是1
    # review 这里虽然不改代码，但是含义变了，现在的最后一个variable就是输出variable
    lp_f[2] = 1
    ...
    m1.setMObjective(None, lp_f, 0.0, None, None, all_vars, GRB.MINIMIZE)
    c2 = m1.objVal
    print("{0} {1} {2}".format(all_vars[0], all_vars[1], all_vars[2]))

    if c1 <= 3.0+epsilon and c2 >= 1.0+epsilon:
        ok = 1
    else:
        ok = 0
    return ok, c1, c2
