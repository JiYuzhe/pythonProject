...
data_file_path = '../../data/points_3.txt'
...
def custom_loss(y_true, y_pred):
    ...
    print(y_true.shape)
    c1, c2, c3 = tf.constant(1.0, dtype=float), tf.constant(1.0, dtype=float), tf.constant(0.1, dtype=float)
    ...
    # TODO 这里面是四个0还是五个0可以再叙
    # total_loss = c1 * 100 * (1 - y_true_label) * tf.math.maximum(y_pred, 0) + c2 * 100000 * tf.math.maximum(0.0001 - tf.abs(
    #     y_true_input[:, 1] - y_true_input[:, 2]), 0) * y_true_label * tf.math.maximum(-y_pred, 0)
    # total_loss = c1 * (1.0 - y_true_label) * tf.math.maximum(y_pred, 0)
    # tf.abs(tf.maximum(-tf.log((tf.abs(output_invariant) + 0.00001)), 0))
    total_loss = c1 * (1 - y_true_label) * tf.math.abs(tf.math.maximum(-y_pred, 0)) + \
                 c2 * y_true_label * tf.math.abs(tf.math.maximum(y_pred, 0)) + \
                 c3 * tf.math.maximum(-tf.math.log(tf.math.abs(y_pred) + 0.0001), 0)
...
def model_builder(hp):
    ...
    lr_schedule = tf.keras.optimizers.schedules.InverseTimeDecay(initial_learning_rate=0.1,
                                                                 decay_steps=50,
                                                                 decay_rate=5,
...
tuner = kt.Hyperband(hypermodel=model_builder, objective='val_loss',
                     max_epochs=10, factor=3, directory='../my_dir', project_name='choose_hyper3')
...
#  is {best_hps.get('units6')}.
# """)
print(f"""
The hyperparameter search is complete. The optimal number of units in the first densely-connected
layer is {best_hps.get('units1')} and the optimal number of units in the second densely-connected layer
 is {best_hps.get('units2')} and the optimal number of units in the second densely-connected layer
 is {best_hps.get('units3')} and the optimal number of units in the second densely-connected layer
 is {best_hps.get('units4')}
""")

# review 首先挑选epoch超参数
model = tuner.hypermodel.build(best_hps)
# model.load_weights('../../checkpoints/weight3')
history = model.fit(train_dataset, validation_data=validation_dataset, epochs=3000)
model.save_weights('../../checkpoints/weight3')
# model.load_weights('../../checkpoints/weight3')
loss = model.evaluate(test_dataset)
print(model.predict([[9.825, 3.5666666666666664, 3.566580952380952], [9.825, 3.0, 3.0001],
                     [6.156738151295698, 2.4812776118704134, 2.481277524037911]
                        , [9.959579895056049, 1, 4.979789947528024],
                     [5.9106000000000005, 2.431172595682777, 2.4311725566072027]]))
# n_hidden = [3, 24, 4, 1]
# net_dir = "../../net/ex3/"
